...
3. Install the hooks: `bun scripts/cursor-install.ts`. The installer idempotently merges Correction Guy's sessionStart, preToolUse, postToolUse, and stop entries into `~/.cursor/hooks.json` with absolute paths, preserving everything else in that file. Re-run it after every `git pull` so new hook entries land.
