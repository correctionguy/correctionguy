...
4. **Remember.** Keep memory in project-local `.memory` so it lives with repo. Real files in `.memory`; traditional memory dir = symlink into it. `.memory` missing or layout wrong (incl after rename) -> run correctionguy:setup skill (host lacks it -> move files into `.memory`, symlink old path to it). User corrects you, you learn non-obvious thing, or owner states fact (a User's Claim) -> record to `.memory` (default home for every memory; elsewhere only if rule says so); treat User's Claims as sourced.
