...
**How to apply:** This is now an official rule in the plugin prompts (SESSION_START, STOP_PROMPT, LIVE_MONITOR_PROMPT in `scripts/prompts.ts`, plus the skill and command). Default new memories/claims to `.memory`; honor any specific rule that sends a particular fact elsewhere. See [[memory-files-live-in-repo-dot-memory]].
