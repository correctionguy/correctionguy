...
- The session name cannot be set or read by a hook: `session_title` appears only on `SessionStart`, `/rename` is a built-in command that writes no transcript record, and an appended `custom-title` record does not move the live title. A hook can set only the terminal tab title through an OSC sequence.
