...
- **Cursor:** exposes neither todos nor chat title to hooks — not in any payload, not in the transcript. They degrade to empty. Since the 2026-07-23 guidance change, hosts with no native tracker keep the TODO list in `.memory/TODO.md`; the live monitor reads that file when `todos` is empty, and the stop check (no `todos` input) reads it when the transcript shows no native tracker in use. See [[todo-list-fallback-dot-memory]].
