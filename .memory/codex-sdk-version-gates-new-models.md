...
- Repeated 2026-09-07: SDK 0.144.2 400-rejects `gpt-6-astra`; SDK 0.153.4 passes the smoke test. The caret range in `package.json` keeps `bun update` on 0.144.x, so the cache stopgap is `bun add @openai/codex-sdk@<version> --ignore-scripts` in the cache dir (`--ignore-scripts` skips the `prepare` lefthook install). The next `claude plugin update` replaces the cache, so the repo needs the same bump. Done 2026-09-14 (v3.20.0): `package.json` pins `@openai/codex-sdk` 0.153.4 exactly and the `scripts/codex.ts` effort enum mirrors that SDK's union. See [[codex-effort-levels-and-speed-tiers]] and [[claude-code-env-block-hot-applies]].
