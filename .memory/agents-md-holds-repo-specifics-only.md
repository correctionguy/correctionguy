...
**How to apply:** Before adding to AGENTS.md, ask which layer owns the fact. General discipline goes to the global file, background to `.memory`, and only a repo-specific trap stays here. Verified detail that makes the split safe: the reviewer prompts in `scripts/prompts.ts` name the instruction files per host and carry no repo example (the helper-function-ban example was removed 2026-09-14), and the reviewer carries its own six-failure ruleset inline ([[reviewer-six-failures]]). Since 2026-09-14 the Claude host's list also names `~/.claude/CLAUDE.md` and `~/.claude/rules/`, so the Codex reviewer subprocess can read the owner's global rules; a rule stays in AGENTS.md only when it is repo-specific. Do not "restore" deleted general-discipline rules on the theory that the reviewer needs them. See [[high-entropy-memory-mining]], [[release-process]], [[pi-extension-integration]].
