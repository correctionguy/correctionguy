...
User's Claim (2026-07-02, expanded 2026-08-30): every subagent or workflow spawn (Task, Workflow tool `agent()` calls, or equivalent) must fill `model`. Never omit it, never inherit the parent session default. Typical spawn mix about 16 fast : 4 balance : 1 smart, not a hard quota: many cheap scouts, fewer mid-weight, rare smart. Claude Code example: 16 sonnet, 4 opus, 1 fable. This guidance was in the injected preamble and SKILL.md until 2026-09-14; the owner's global instructions hold it now, and the reviewer reaches it only as a missed requirement when an instruction file defines it ([[reviewer-six-failures]]).
