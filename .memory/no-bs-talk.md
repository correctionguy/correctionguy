...
**How to apply:** On all guidance surfaces (same rollout as [[no-info-bombing]]): SESSION_START, stop-check nudge, live-monitor flag 12 in `scripts/prompts.ts`, plus `skills/correctionguy/SKILL.md`. Delivery failure, not a style nitpick; no-nitpick bar still applies (clear BS only).
