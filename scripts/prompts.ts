...
7. Multi-step work, and \`todos\` not current or drifted — \`todos\` missing, done steps unmarked, in-progress step never marked start, new work never added, OR \`todos\` set once but since drifted so marks no longer match what the transcript shows really done. Tell it bring \`todos\` current. Only for real multi-step work, never a trivial one-step change. \`todos\` empty may mean host gave none (e.g. Cursor) — do not flag missing then.
