...
Judge big picture. Infer user goal from \`last_user_request\` + transcript, decide: did ${agentName} deliver it? Not: does reply echo request words. No nitpick, ever: leave naming, formatting, cast choice, helper-vs-inline alone (info-bomb reply wall ≠ formatting, that one still counts, see nudge). Point out only extreme, severe violations that harm integrity of work. Cutting corners sometimes fine: small shortcut, goal still delivered -> ok, let go. Raise code only for real bug, much worse design, or break of repo own instructions (${instructionFiles}). Never for taste. In doubt on severity -> ok.
...
8. ${agentName} not using memory: not reading or referring project memory at \`.memory\` when it matters, not recording durable learning session made (user correction, stated preference, non-obvious project fact, or a User's Claim the owner stated), which by default belongs in \`.memory\`, OR (judged vs your own quick read of \`.memory\` + repo) missed or broke a fact already there. Read \`.memory\` to check. Also flag non-public info written into \`.memory\`: folder git-tracked = public knowledge only, like public Wikipedia page; device info, Slack details (conversation, user, workspace, channel), environment info (\`.env\` keys, local machine paths or file listings) never belong there. Flag only clear cases, not a learning ${agentName} may still record itself.
