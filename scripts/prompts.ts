export const SESSION_START =
  "Recall memory. Restate task. List key memories + tools in use. Memory live here: real files in `<project>/.memory`. Traditional memory dir (like `~/.claude/projects/<slug>/memory`) = symlink into `.memory`, not the reverse. Real files still in traditional dir, or a rename moved them? Move into `<project>/.memory`, then symlink old path to it before you go on. User corrects you, or you learn non-obvious thing about project -> write to `.memory` now. `.memory` is the default home for every memory; put one elsewhere only if a rule says so. Owner/user states a fact about the project (a User's Claim) -> record it to `.memory` and treat it as sourced: never flag a recorded claim unverified, never delete it for lacking a link. Explore project. List every outside thing you touch: APIs, CLIs, platforms, library behavior. Your knowledge stale -> never guess from memory. Verify third-party with web search or official docs before edit. Multi-step task -> keep todo list, keep current: grab steps up front, mark each start when you begin + done the moment you finish, add new as they show, use whatever task tracker the session gives (TodoWrite, task list, whatever). User piles on many asks, or keeps adding or repeating requests across the session -> log each as its own todo right away, never just hold them in your head; memory drops asks, a tracked list does not. Do full work asked. No stub, no hidden cut. Run code before stop. Cannot -> say what blocked.";
...
7. Multi-step work — or user stacked several asks, or kept adding or repeating requests across the session — and \`todos\` not current or drifted — \`todos\` missing (incl piled-on asks Claude only holds in its head, never logged — memory drops asks), done steps unmarked, in-progress step never marked start, new work or fresh asks never added, OR \`todos\` set once but since drifted so marks no longer match what the transcript shows really done. Tell it bring \`todos\` current. Only for real multi-step work or a real pile of asks, never a trivial one-step change. \`todos\` empty may mean host gave none (e.g. Cursor) — do not flag missing then.
