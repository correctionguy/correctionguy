...
- The lefthook pre-commit gate runs all four commands including the metered `bun run test`, so wherever it is installed **every** `git commit` (even docs-only) fails until Codex is authenticated as above. It does not fire in this Cursor Cloud checkout (see the install bullet: `core.hooksPath` is pinned and `prepare` was skipped), so a `git commit` here succeeds without it. Run the four commands by hand, and never bypass the gate where it is active.

### Co-installed sibling repo: ponytail

A second, unrelated repo (`dietrichgebert/ponytail`) is set up in this environment alongside correctionguy, per an explicit request. It lives at `$HOME/ponytail`, deliberately outside `/workspace` so its checkout never pollutes this repo's git tree or `ultracite check`. The update script keeps it fresh: guard-clone if missing, `npm install --prefix "$HOME/ponytail/ponytail-mcp"`, and `pip3 install --user pandas`. It is a separate git repo; do not commit its changes onto a correctionguy branch.

- Ponytail is Node + stdlib Python (no lockfile; npm). Only `ponytail-mcp` has npm deps (`@modelcontextprotocol/sdk`, `zod`); the top-level and `pi-extension` have none, and the Hermes plugin (`__init__.py`) is stdlib-only.
- Run its full suite with `npm test` from `$HOME/ponytail` (top-level `node --test tests/*.test.js` plus `pi-extension` and `ponytail-mcp`). The `csv` case in `tests/correctness.test.js` shells out to `python3` and imports `pandas`, so it is the one test that fails without pandas installed (`pip3 install --user pandas`); everything else needs no extra deps.
- Core entry points to exercise it headless: the MCP stdio server `node ponytail-mcp/index.js` serves a `ponytail_instructions` tool and `ponytail` prompt (drive it with an MCP stdio client); the Python `build_injected_context(mode)` in `__init__.py` returns the mode-filtered ruleset (`off` yields an empty string).
